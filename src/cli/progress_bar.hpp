...
namespace cpm {
	...
	class ProgressBar {
		...
		/**
		 * @brief Constructor for progress bar. This creates the first stage of
		 *		  the new progress bar and sets it as the active stage.
		 * 
		 * @param title the title of the first stage
		 * @param max_value the maximum value of the first stage progress bar.
		 * 		  By default it is 0, which means that the maximum value is
		 ...
		 * @param max_value the maximum value of the new stage progress bar.
		 * 		  By default it is 0, which means that the maximum value is
		 * 		  unknown. In that case the progress bar circles through
		 * 		  infinitely to simulate progress.
		 */
		...
		/**
		 * @brief Fill the current active progress bar up to the value specified
		 * 		  by this->active_stage.value (or circle through to simulate
		 * 		  progress).
		 */
		void fill_bar();
	};
